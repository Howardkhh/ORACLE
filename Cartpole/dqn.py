...
def save_model(model_path, q_network, run_name, args: Args, device, writer):
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.checkpoint_frequency == 0:
                ...
                episodic_returns = save_model(model_path, q_network, run_name + f"-{global_step}", args, device, writer)
                mean_reward = (sum(episodic_returns)/len(episodic_returns))[0]
                print(f"Saved model checkpoint to {model_path}, reward: {mean_reward}")
                evaluation_rewards[global_step] = mean_reward

    if args.save_model:
        model_path = f"runs/{run_name}/{args.exp_name}.cleanrl_model"
        save_model(model_path, q_network, run_name, args, device, writer)

    with open(f"runs/{run_name}/evaluation_rewards.txt", "w") as f:
        for step, reward in evaluation_rewards.items():
