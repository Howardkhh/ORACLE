...
class CausalPFNCartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    """
    ## Description
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        with np.load(f"data/CartPole-v1/random_remapped.npz") as data:
        ...
        with np.load(f"data/CartPole-v1/dqn_remapped.npz") as data:
        ...
        context_idx = np.random.choice(np.count_nonzero(done==0), size=100, replace=False)
        target_names = ["Reward", "Cart_Position", "Cart_Velocity", "Pole_Angle", "Pole_Angular_Velocity", "Done"]
        self.causalpfns = []
        for i, name in enumerate(target_names):
            self.causalpfns.append(CATEEstimator(
                device=device,
                # num_neighbours=32,
            ))
            ...
            self.causalpfns[i].fit(X_i, T_i, Y_i)
        
        print("CausalPFN CartPole Env: CausalPFNs trained!")

        # Angle at which to fail the episode
        # self.theta_threshold_radians = 12 * 2 * math.pi / 360
        # self.x_threshold = 2.4
        self.theta_threshold_radians = 8 * 2 * math.pi / 360
    ...
    def step(self, action):
        ...
        reward_outcome= self.causalpfns[0].estimate_cate(self.state[np.newaxis, :])
        reward = reward_outcome[1:][action][0]
        self.state = np.array([outcome[1:][action][0] for outcome in [self.causalpfns[i].estimate_cate(self.state[np.newaxis, :]) for i in range(1, 5)]])
        x, x_dot, theta, theta_dot = self.state
...
class CausalPFNCartPoleVectorEnv(VectorEnv):
    metadata = {
        "render_modes": ["human", "rgb_array"],
        "render_fps": 50,
    }
    ...
    def __init__(
        ...
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        context_idx = np.random.choice(np.count_nonzero(done==0), size=10000, replace=False)
        target_names = ["Reward", "Cart_Position", "Cart_Velocity", "Pole_Angle", "Pole_Angular_Velocity", "Done"]
        self.causalpfns = []
        for i, name in enumerate(target_names):
                self.causalpfns.append(CATEEstimator(
    ...
    def step(
        ...
        reward_outcome= self.causalpfns[0].estimate_cate(self.state[np.newaxis, :])
        reward = reward_outcome[1:][action][0]
        self.state = (outcome[1:][action][0] for outcome in [self.causalpfns[i].estimate_cate(self.state[np.newaxis, :]) for i in range(1, 5)])
        x, x_dot, theta, theta_dot = self.state
