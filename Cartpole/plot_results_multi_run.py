...
class Args:
    log_folder: str = "logs/"
    env_id: str = "cartpole"
    methods: tuple[Literal["dqn", "random", "mix"], ...] = ("random", "mix")
    ...
    num_samples: tuple[int, ...] = (200, 400, 600, 800, 1000)
    num_runs: int = 10
                ...
                for oracle in args.oracle:
                    for source in args.oracle_source:
                        for run in range(args.num_runs):
                            records = reward_record[method][ratio][oracle][source][num_sample]
                            steps = sorted(records.keys())
                            # (num_steps, num_runs)
                            rewards = ema(np.array([records[step][run] for step in steps]))
                            ...
                            if oracle == "no":
                                ...
                                color = "blue"
                                records_double_data = reward_record[method][ratio][oracle][source][num_sample * 2]
                                steps_double_data = sorted(records_double_data.keys())
                                rewards_double_data = ema(np.array([records_double_data[step][run] for step in steps_double_data]))
                                df_double_data = pd.DataFrame({
                                    "Step": steps_double_data,
                                ...
                                sns.lineplot(
                                    ...
                                    x="Step",
                                    y="Total Reward",
                                    label=f"{method}{ratio} - oracle: {oracle} (double data)",
                                    color="orange",
                                    # linestyle="--",
                                    errorbar="sd"
                                )
                            ...
                            sns.lineplot(
                                ...
                                x="Step",
                                y="Total Reward",
                                label=f"{method}{ratio} - oracle: {oracle}",
                                color=color,
                                errorbar="sd"
                            )
                plt.title(f"CartPole-v1 Offline DQN Learning Curve (data={method}{ratio} policy, oracle source={source}, num_samples={num_sample})")
                plt.xlabel("Steps")
                plt.ylabel("Episodic Return")
                plt.ylim(0, 550)
                plt.legend(loc="upper left").set_visible(False)
                plt.grid()
                plt.savefig(f"outputs/training_results/cartpole_offline_dqn_method_{method}{ratio}_oracle_source_{source}_num_samples_{num_sample}_multi_run.png")
                plt.close()